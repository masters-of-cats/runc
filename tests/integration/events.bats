...
@test "events --interval test_busybox test_busybox_disappearing (should exit when any container dies)" {
  # XXX: currently cgroups require root containers.
  requires root
  ...
  runc run -d --console-socket $CONSOLE_SOCKET test_busybox_disappearing
  [ "$status" -eq 0 ]

  (__runc events --interval 100ms test_busybox test_busybox_disappearing > events.log && echo $? > runc_exitcode) &

  retry 3 1 eval "grep -q 'test_busybox_disappearing' events.log"
  teardown_running_container test_busybox_disappearing
